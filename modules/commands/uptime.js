...
var time = process.uptime(),
  ...
  let k = args[0];
   if(args[0] == "list"){
    const alime = (await axios.get('https://raw.githubusercontent.com/RUDRO-TEAM-404/ArYAN/refs/heads/main/dataanime.json')).data
  ...
  var id = Math.floor(Math.random() * 848) +1
    ...
    const lengthchar = (await axios.get('https://raw.githubusercontent.com/RUDRO-TEAM-404/ArYAN/refs/heads/main/dataanime.json')).data
    console.log(lengthchar.length)
    const Canvas = require('canvas');
    let pathImg = __dirname + `/tad/${id}.png`;
    let pathAva = __dirname + `/tad/${event.senderID}.png`;
    let background = (await axios.get(encodeURI(`https://raw.githubusercontent.com/RUDRO-TEAM-404/ArYAN/refs/heads/main/x5JpRYu.png`), { responseType: "arraybuffer" })).data;
...
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "55px time";
    ctx.fillText("bayjid_404", 930, 540)
    ctx.fillText("BaYjid", 930, 610)
    ctx.fillText("bayjid.x.404", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
